...
- **7 MarketInputs labels migrated from `<Tip>` to `<GlossaryLink>`** — Mirrors the v3.9.99.12 migration for data-table headers. Labels Mtg Rate, Invest Return, Appreciation, Rent Growth, Prop Tax, Insurance, Maintenance now show a `cursor:help` + browser `title` tooltip for a quick first-sentence reminder on desktop, and tap/click jumps to the full Glossary entry on both desktop and mobile. Applied to **both** ScenarioPanel instances: the "Your Assumptions" panel (cusOvr state) and the "What-If" panel (scenOvr state). Both panels are now consistent with the data-table UX, and both panels stay independent — `gl` is a stateless navigation helper with no panel-specific capture, so there's no ghost linking between cusOvr and scenOvr values.
- **`GlossaryLinkBase` lifted to module scope** — Previous v3.9.99.12 `GlossaryLink` was defined inside `RentVsBuyV3` as a closure over `setGuideTab` / `guideRef` / `setMethodHighlight`. Module-level components like `NumInput` and `MarketInputs` couldn't reach it. Restructured: `GlossaryLinkBase` lives at module scope and accepts the three dependencies as explicit props. Inside `RentVsBuyV3`, a thin `gl` alias pre-binds the current-render values of those three, and the existing `GlossaryLink` name aliases to `gl` so the data-table header call site (the v3.9.99.12 caller) is a zero-diff rename.
- **Prop drilling via `gl` — 3 levels, no new React machinery** — The `gl` alias travels `RentVsBuyV3` → `ScenarioPanel` → `MarketInputs` → `NumInput` as a single prop named `gl`. Intermediates add `gl` to their signature and forward it; they don't read it. `NumInput` destructure-renames `{gl:Gl}` so JSX can use capitalized `<Gl term={label}>` directly (lowercase JSX tags are treated as HTML elements). `NumInput` renders `<Gl>` when passed, `<Tip>` when not — preserving backward compatibility for the 7 "Your Situation" NumInputs that still use `<Tip>` (those stay as-is per scope decision; the Your Situation input set is expected to change during v4 budget-model work, so migrating them now would create rework). Chose prop drilling over `React.createContext` — adding a Context for one consumer is YAGNI; if future work adds more consumers, that's the right trigger to introduce Context.
- **7 dead `TIPS` entries pruned** — `mortgageRate`, `investReturn`, `appreciation`, `rentGrowth`, `propTaxRate`, `insRate`, `maintRate` are now defined in `GLOSSARY` (short hint derived from the first sentence of `def`). Removing them from `TIPS` closes the redundancy loop. `TIPS` now holds 7 entries, all for Your Situation inputs still using `<Tip>`. Matches the v3.9.99.11→.12 pattern where `TABLE_TIPS` was pruned as entries became obsolete.
- **`<Tip>` component retained** — 7 active callers (Your Situation inputs: rentBudget, buyBudget, downPayment, mortgageTerm, holdingPeriod, simYears, discipline). Full retirement of `<Tip>` is deferred until those inputs either get Glossary entries or get replaced by the v4 budget-first architecture. Explicit scope decision per planning — shipping content first (v4 may restructure these inputs entirely) is cheaper than migrating and then re-migrating.
- **Structural diff summary** — Net +14 lines (GlossaryLinkBase at module scope ≈ 22 lines, minus the ~25 lines removed from the inside-RentVsBuyV3 closure version, plus 3-line gl alias, plus 1 prop on ScenarioPanel signature, plus 1 prop on MarketInputs signature, plus 7 prop forwards + 2 ScenarioPanel forwards + 1 MarketInputs forward, plus 3-line {Gl?...:<Tip>...} ternary). 7 TIPS entries deleted. Engine, `simulate`, `buildParams`, and all chart code untouched.
...
- **Glossary tab restructured with two nested `<details>`** — New sections "Data-Table Columns" (15 entries) and "Assumption Input Definitions" (7 new). Both default **collapsed** — users see two labeled summary bars on tab open and choose which to expand. Short intro paragraph renders under each summary describing the section's contents. Section summaries match the project's existing pattern (cursor:pointer, flex row with ▶ icon + bolded label).
- **Arrow rotation scoped per-details** — CSS selector tightened from `details[open] .disclosure-icon` to `details[open] > summary .disclosure-icon`. Without the direct-child constraint, any inner `.disclosure-icon` inherited the rotation of an open ancestor, so nested section arrows stayed pointing down even when their own section was closed. The `>` means each arrow now only responds to its own details' state, matching the logic the outer Guide arrow already had in isolation.
- **Accordion behavior — one section open at a time** — New module-level `accordionToggle` handler wired via `onToggle` on both Glossary sections. When a section opens, it closes all its sibling `<details>` at the same DOM level via `:scope > details`. Generalized — works for any number of sibling sections, so future v4 Glossary additions (market concepts, Expert-mode inputs) drop in with the same handler, no per-section logic. Handler returns early on close events so programmatic closes don't trigger cascade.
- **Reset-on-Guide-open** — New module-level `resetNestedDetails` handler wired via `onToggle` on the outer Guide `<details>`. When the Guide opens (including reopen after being closed, and programmatic opens from `GlossaryLink`), it force-closes all nested details inside it, so every Guide-open starts from a known clean collapsed state. Generalized — applies to any future nested details (FAQ sub-accordions, Methodology subsections) without additional wiring. Triggering on open rather than on close sidesteps a WebKit layout-cache bug where collapsing nested details while the parent was hiding left stale expanded-height space reserved for the next reopen paint.
- **`GlossaryLink` click handler walks ancestor `<details>`** — After finding the target entry element, loops up via `parentElement` and sets `open=true` on any `<details>` ancestor between the entry and `guideRef.current`. This force-opens the inner section (Data-Table Columns or Assumption Input Definitions) if it was collapsed, so jump-and-highlight always reveals the target. Stops at `guideRef.current` because that outer `<details>` is opened separately earlier in the handler. Six new lines of handler logic, zero new state. Because the inner section opens programmatically, `accordionToggle` fires and correctly closes the sibling section — the jump always lands on a clean single-section view.
...
- **Structural diff summary** — Net +24 lines JSX (two `<details>` wrappers × 12 lines each), +7 GLOSSARY array entries, +1 lookup condition (`g.aliases?.includes`), +6 handler lines (ancestor walk), +2 module-level helpers (`accordionToggle`, `resetNestedDetails`), CSS selector tightened (1 line). Engine, `simulate`, `buildParams`, and all chart code untouched — this is content + UI wrapping, not behavior.
