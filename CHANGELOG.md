...
- **Reset-on-Guide-close** — New module-level `resetNestedDetails` handler wired via `onToggle` on the outer Guide `<details>`. When the Guide collapses, it force-closes all nested details inside it, so reopening the Guide returns Glossary subsections to their default collapsed state. Generalized — applies to any future nested details (FAQ sub-accordions, Methodology subsections) without additional wiring.
