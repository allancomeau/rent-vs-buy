...
- [ ] **Inflation ↔ rent-growth default-tie** with unlink affordance — decide during budget-model scoping. Historically rent growth ≈ inflation; a linked default with explicit unlink (same pattern as Rent/P&I link toggle) would reduce scenario-input burden for beginners while preserving Expert flexibility. Parked here because the resolution depends on whether v4 surfaces inflation as a direct input or keeps it as a derived/hidden parameter.
