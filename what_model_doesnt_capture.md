...
### Metro Selection Bias
- **What it is:** The 54 metros are primarily major economic centers (MSA-level). Larger metros tend to have higher price-to-rent ratios, which structurally favors renting in the Location panel.
- **Why it exists:** Metro-level data (appreciation, rent growth, property tax, insurance) is only reliably sourced for larger MSAs. Smaller cities and rural areas lack consistent public data at the same granularity.
- **Mitigation:** The Custom panel accepts any values — users in smaller markets can enter their actual local rates. The National Average data point blends expensive and affordable areas. The verdict-driving inputs (payment, down payment, investment return, discipline) are user-controlled and independent of metro selection.
- **Directional bias:** Location panel slightly favors renting due to metro skew. Custom panel is unaffected.
